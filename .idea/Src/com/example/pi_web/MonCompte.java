...
public class MonCompte extends MonCompte_design implements View {
	...
	public MonCompte() {
		...
		button_retour.addClickListener(
			new ClickListener() {
		...
		button_confirmer.addClickListener(
				new ClickListener() {
					public void buttonClick(ClickEvent event) {
						...
						if(!tel.equals(currentUser.getTelephone()) || !mail.equals(currentUser.getEmail()) || mdpNouveau.length() > 0) {
							...
							if(mdpNouveau.length() > 0 || !mail.equals(currentUser.getEmail()))
									mdpRequis = true;
							String erreurMsg = "";
							...
							if(erreurMsg.length() == 0) {
								...
								Notification.show(erreurMsg, Notification.TYPE_WARNING_MESSAGE);
						} else
